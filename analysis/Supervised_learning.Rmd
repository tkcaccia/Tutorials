...
```{r, echo=TRUE}
    ...
    pp$Ypred
```
    pp=pls(Xtrain,Ytrain,Xtest,proj=TRUE)

    plot(pp$Ttrain,col=Ytrain)
    points(pp$Ttest,col=Ytest,pch=2)

\begin{figure}[h]
  \centerline{\includegraphics[width=12cm]{PLSplot.pdf}}
  \caption{Projection of the test data on the training data plot}
  \label{fig:F1}
\end{figure}
...
R2Y: Measures how well the model explains the response variable (Ytrain).R²Y close to 1 means a good fit while R²Y close to 0 represent model does not explain Y well.
...
Double cross-validation is an approach to model hyperparameter optimization and model selection that attempts to overcome the problem of overfitting the training dataset.
This involves two stages, outer and inner cross-validation. 
